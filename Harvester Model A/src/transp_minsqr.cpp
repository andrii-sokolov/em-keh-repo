...
double progress_optim::minsqr_norm()
{
    ...
    double max_theor_vrms = vrms_interp_progress(1.0,max_sol,2*M_PI*f_max*max_sol);
    /** Min square method */
    double sol_1;
    double vrms_1;
    double Sum = 0.0;
    for(int i=0; i<imax; i++)
    {
        ...
        vrms_1 = vrms_interp_progress(1.0,sol_1,2*M_PI*experimental_freq[i]*sol_1)/max_theor_vrms;
...
double progress_optim::minsqr()
{
    ...
    for(int i=0; i<imax; i++)
    {
        ...
        vrms_1 = vrms_interp_progress(scale,sol_1,2*M_PI*experimental_freq[i]*sol_1);
...
double progress_optim::minsqr_sets()
{
    int n_sol;
    double Sum = 0.0;
    double* sol = new double [10];
    ...
    for(int i=0; i<imax; i++)
    {
        ...
        for(int j=0; j<n_sol; j++)
        {
            vrms_1[j] = vrms_interp_progress(scale,sol[j],2*M_PI*experimental_freq[i]*sol[j]);
        }
    ...
    for(int i=0; i<imax1; i++)
    {
        ...
        for(int j=0; j<n_sol; j++)
        {
            vrms_1[j] = vrms_interp_progress(scale,sol[j],2*M_PI*experimental_freq_1[i]*sol[j]);
        }
    ...
    for(int i=0; i<imax2; i++)
    {
        ...
        for(int j=0; j<n_sol; j++)
        {
            vrms_1[j] = vrms_interp_progress(scale,sol[j],2*M_PI*experimental_freq_2[i]*sol[j]);
        }
